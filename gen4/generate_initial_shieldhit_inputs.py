...
console_message = input(f"Will generate batch {BATCH_NUM} in {SAVE_DATA_LOCATION} continue? (y)/n ")
if console_message == "n":
    sys.exit(0)
...
def generate_inputs(inputs):
    ...
    with open(Path(HOME, "templates", TEMPLATE_BEAM), "r") as f:
    ...
    with open(Path(HOME, "templates", TEMPLATE_GEO),"r") as f:
        new_geo_file = f.read()
    new_geo_file = new_geo_file.format(cyl_height=cyl_height)

    with open(Path(HOME, "templates", TEMPLATE_DET),"r") as f:
...
def copy_templates(target_dir):
    ...
    shutil.copyfile(Path(HOME, "templates", TEMPLATE_MAT), Path(target_dir,"mat.dat"))
